...
fn lbm() {
    ...
    while !win.is_closed() && iter < maxiter {
        ...
        if iter % 10 == 0 {
            ...
            win.draw_image(
                &flip(&transpose(&normalize(&uu), false), 0).cast::<f32>(),
            ...
            win.draw_vector_field2(
                &flat(&view!(x[filter,filter])).cast::<f32>(),
                &flat(&view!(y[filter,filter])).cast::<f32>(),
                &flat(&view!(ux[filter,filter])).cast::<f32>(),
                &flat(&view!(uy[filter,filter])).cast::<f32>(),
